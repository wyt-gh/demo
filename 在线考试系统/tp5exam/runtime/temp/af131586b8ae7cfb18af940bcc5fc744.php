<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:86:"D:\phpstudy\PHPTutorial\WWW\tp5exam\public/../application/admin\view\single\index.html";i:1603271024;s:75:"D:\phpstudy\PHPTutorial\WWW\tp5exam\application\admin\view\Common\head.html";i:1602636435;}*/ ?>
<!DOCTYPE html>
                    ...
                    <div class="layui-card">
                        <div class="layui-card-body ">
                            <form class="layui-form layui-col-space5">
                                ...
                                <div class="layui-inline layui-show-xs-block">
                                    <input class="layui-input" placeholder="请输入编号" name="id">
                                </div>
                                <div class="layui-inline layui-show-xs-block">
                        ...
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加用户','<?php echo url('Single/add'); ?>',800,600)"><i class="layui-icon"></i>添加</button>
                        </div>
                              ...
                              <tbody>
                                  <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                	<tr>
	                                    ...
	                                    <td><?php echo sub_name($vo['single_name']); ?></td>
	                                    <td><?php echo $vo['A']; ?></td>
                        				      <td><?php echo $vo['B']; ?></td>
	                                    <td><?php echo $vo['C']; ?></td>
	                                    <td><?php echo $vo['D']; ?></td>
                                      ...
                                      <a onclick="member_stop(this,'<?php echo $vo['id']; ?>')" href="javascript:;"  title="<?php if($vo['status'] == '1'): ?>正常<?php else: ?>停用<?php endif; ?>">
	                                    ...
	                                    <td class="td-manage">
		                                    ...
		                                    <a title="编辑"  onclick="xadmin.open('编辑','<?php echo url('Single/edit',['id'=>$vo['id']]); ?>',600,400)" href="javascript:;">
    ...
    <script>
      ...
      function member_stop(obj,id){
        if($(obj).attr('title')=='正常'){
          //发异步把用户状态更改为停用
          $.post("<?php echo url('Single/off'); ?>",{id:id},function(data) {
            if (data.code == 1) {
              $(obj).attr('title','停用');
          ...
          //发异步把用户状态更改为启用
          $.post("<?php echo url('Single/on'); ?>",{id:id}, function(data) {
            if (data.code == 1) {
              $(obj).attr('title','正常')
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              $.post("<?php echo url('Single/del'); ?>",{ids:id},function(data) {
                console.log(data)
                if (data.code == 1) {
                  //删除成功
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？'+ ids.toString(),function(index){
          $.post("<?php echo url('Single/del'); ?>",{ids:ids},function(data) {
            if (data.code == 1) {
              //删除成功
              layer.msg(data.msg, {icon: 1});
