<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:84:"D:\phpstudy\PHPTutorial\WWW\tp5exam\public/../application/admin\view\book\index.html";i:1606717341;s:75:"D:\phpstudy\PHPTutorial\WWW\tp5exam\application\admin\view\Common\head.html";i:1602636435;}*/ ?>
<!DOCTYPE html>
                ...
                <div class="layui-col-md12">
                    <div class="layui-card">
                        ...
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加用户','<?php echo url('Book/add'); ?>',800,600)"><i class="layui-icon"></i>添加</button>
                        </div>
                                  ...
                                  <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                                	<tr>
	                                    ...
	                                    <td><?php echo $vo['book_name']; ?></td>
                        				      <td>
                                        <img src="<?php echo $vo['cover_img']; ?>" style="width: 90px;height: 120px">
                                      </td>
	                                    <td><?php echo $vo['grade']; ?></td>
	                                    <td><?php echo $vo['read']; ?></td>
	                                    <td><?php echo sub_name($vo['synopsis']); ?></td>
                                      <td><?php echo !empty($vo['vip'])?'是':'否'; ?></td>
	                               		  <td><?php echo $vo['major_name']; ?></td>
                                      <td><?php echo $vo['subject_name']; ?></td>
	                                    <td><?php if($vo['end'] == '1'): ?>是<?php else: ?>否<?php endif; ?></td>
                                      <td class="td-status">
                                      ...
                                      <td>
                                        <button class="layui-btn" onclick="xadmin.open('添加章节','<?php echo url('Book/addsection',['id'=>$vo['id']]); ?>',1000,800)">
                                          <i class="layui-icon"></i>添加章节
                                        </button><br>
                                          <button class="layui-btn" onclick="xadmin.open('修改章节','<?php echo url('Book/editsection',['id'=>$vo['id']]); ?>',1400,800)">
	                                    ...
	                                    <td class="td-manage">
		                                    ...
		                                    <a title="编辑"  onclick="xadmin.open('编辑','<?php echo url('Book/edit',['id'=>$vo['id']]); ?>',800,600)" href="javascript:;">
    ...
    <script>
      ...
      function member_stop(obj,id){
        if($(obj).attr('title')=='正常'){
          //发异步把用户状态更改为停用
          $.post("<?php echo url('Book/off'); ?>",{id:id},function(data) {
            if (data.code == 1) {
              $(obj).attr('title','停用');
          ...
          //发异步把用户状态更改为启用
          $.post("<?php echo url('Book/on'); ?>",{id:id}, function(data) {
            if (data.code == 1) {
              $(obj).attr('title','正常')
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              $.post("<?php echo url('Book/del'); ?>",{ids:id},function(data) {
                console.log(data)
                if (data.code == 1) {
                  //删除成功
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？'+ ids.toString(),function(index){
          $.post("<?php echo url('Book/del'); ?>",{ids:ids},function(data) {
            if (data.code == 1) {
              //删除成功
              layer.msg(data.msg, {icon: 1});
