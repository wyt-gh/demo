<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:85:"D:\phpstudy\PHPTutorial\WWW\tp5exam\public/../application/admin\view\major\index.html";i:1603697441;s:75:"D:\phpstudy\PHPTutorial\WWW\tp5exam\application\admin\view\Common\head.html";i:1602636435;}*/ ?>
<!DOCTYPE html>
                ...
                <div class="layui-col-md12">
                    <div class="layui-card">
                        <div class="layui-card-header">
                            ...
                            <button class="layui-btn" onclick="xadmin.open('添加用户','<?php echo url('Major/add'); ?>',800,200)"><i class="layui-icon"></i>添加</button>
                        </div>
                                	...
                                	<tr>
                                      ...
                                      <a onclick="member_stop(this,'<?php echo $vo['id']; ?>')" href="javascript:;"  title="<?php if($vo['on_off'] == '1'): ?>正常<?php else: ?>停用<?php endif; ?>">
	                                    ...
	                                    <td class="td-manage">
		                                    ...
		                                    <a title="编辑"  onclick="xadmin.open('编辑','<?php echo url('Major/edit',['id'=>$vo['id']]); ?>',600,400)" href="javascript:;">
    ...
    <script>
      layui.use(['laydate','form'], function(){
        ...
        form.on('select(major)', function(data) {
          ...
          $.post("<?php echo url('Major/major'); ?>",{major_id:major_id},function(data){
            console.log(data)
            //清空subject里面的内容
            $('#subject').empty();
      ...
      function member_stop(obj,id){
        if($(obj).attr('title')=='正常'){
          //发异步把用户状态更改为停用
          $.post("<?php echo url('Major/off'); ?>",{id:id},function(data) {
            if (data.code == 1) {
              $(obj).attr('title','停用');
          ...
          //发异步把用户状态更改为启用
          $.post("<?php echo url('Major/on'); ?>",{id:id}, function(data) {
            if (data.code == 1) {
              $(obj).attr('title','正常')
      ...
      function member_del(obj,id){
          layer.confirm('确认要删除吗？',function(index){
              $.post("<?php echo url('Major/del'); ?>",{ids:id},function(data) {
                console.log(data)
                if (data.code == 1) {
                  //删除成功
      ...
      function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？'+ ids.toString(),function(index){
          $.post("<?php echo url('Major/del'); ?>",{ids:ids},function(data) {
            if (data.code == 1) {
              //删除成功
              layer.msg(data.msg, {icon: 1});
