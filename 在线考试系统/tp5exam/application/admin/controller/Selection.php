...
class Selection extends Common {
	...
	{
		$list = Model('Selection')->getList(1);
		//加工列表
		for ($i=0; $i<count($list); $i++) {
			$m_s = Model('Selection')->get_maj_sub($list[$i]['major_id'],$list[$i]['subject_id']);
			// dump($m_s);die;
	...
	public function edit()
	{
		if (request()->isPost()) {
			...
			$result = db('selection')->where('id',$data['id'])->update($data);
			...
			//部署渲染页面
			$id = input('id');
			$data = db('selection')->where('id',$id)->find();
			// dump($data);die;
	...
	public function add()
	{
		if (request()->isPost()) {
			...
			$result = db('selection')->insert($data);
	...
	public function del()
	{
		...
		$result = Db::name('selection')->where("id in ($ids)")->setField('status',0);
		if ($result) {
			$this->success("删除成功");
		} else {
			$this->error("删除失败");
    ...
    public function off(){
    	...
    	$id = input("post.id");
    	// dump($id);die;
    	$result = Db::name('selection')->where('id',$id)->setField('status',0);
    ...
    public function on(){
    	...
    	$id = input("post.id");
    	// dump($id);die;
    	$result = Db::name('selection')->where("id",$id)->setField('status',1);
