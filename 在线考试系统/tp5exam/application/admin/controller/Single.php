...
class Single extends Common {
	...
	{
		$list = Model('Single')->getList(1);
		//加工列表
		for ($i=0; $i<count($list); $i++) {
			$m_s = Model('Single')->get_maj_sub($list[$i]['major_id'],$list[$i]['subject_id']);
			// dump($m_s);die;
	...
	public function edit()
	{
		if (request()->isPost()) {
			...
			$result = db('single')->where('id',$data['id'])->update($data);
			...
			//部署渲染页面
			$id = input('id');
			$data = db('single')->where('id',$id)->find();
			// dump($data);die;
    ...
    public function off(){
    	...
    	$id = input("post.id");
    	// dump($id);die;
    	$result = Db::name('single')->where('id',$id)->setField('status',0);
    ...
    public function on(){
    	...
    	$id = input("post.id");
    	// dump($id);die;
    	$result = Db::name('single')->where("id",$id)->setField('status',1);
